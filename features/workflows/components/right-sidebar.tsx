...
import {
  ...
  runWorkflowAction,
} from "@/features/workflows/action"
...
function Palette() {
  ...
  const add = (type: NodeType) => {
    ...
    if (
      def.kind === "trigger" &&
      currentNodes.some((n) => n.data.kind === "trigger")
    ) {
      toast.add({
        title: "A workflow can only have one trigger.",
        type: "error",
        timeout: 2000,
      })
      return
    ...
    const count = currentNodes.filter((n) => n.data.type === type).length
